...
export class HomeComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  // Micro text word animation (for words in micro-text elements and SVG tspans)
  startMicroTextWordAnimation() {
    ...
    microTextElements.forEach((word) => {
      const wordEl = word as HTMLElement & SVGElement;
      const isSvg = wordEl.namespaceURI === 'http://www.w3.org/2000/svg';

      // Create random flicker for each word
      const flickerWord = () => {
        const originalColor = isSvg ? wordEl.style.fill : wordEl.style.color;
        ...
        const colorValue =
          flickerType < 0.3
            ? '#ffffff'
            : flickerType < 0.5
              ? ['rgb(148, 163, 233)', 'rgb(197, 205, 243)', 'rgb(244, 245, 253)'][
                  Math.floor(Math.random() * 3)
                ]
              : `rgb(${73 + Math.floor(Math.random() * 180)}, ${73 + Math.floor(Math.random() * 180)}, ${73 + Math.floor(Math.random() * 180)})`;
        const opacityValue =
          flickerType < 0.3
            ? (0.8 + Math.random() * 0.2).toString()
            : flickerType < 0.5
              ? '0.35'
              : (0.35 + Math.random() * 0.65).toString();

        if (isSvg) {
          wordEl.style.fill = colorValue;
        } else {
          wordEl.style.color = colorValue;
        }
        ...
        setTimeout(
          () => {
            if (isSvg) {
              wordEl.style.fill = originalColor;
            } else {
